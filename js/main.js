...
(function ($) {
  ...
  // 37 .scale animation
	...
	if ($('.tp-awards-vp-move-thumbs-wrapper').length > 0) {
		const awardsMM = gsap.matchMedia();

		awardsMM.add("(min-width: 1200px)", () => {
			...
			const moveThumbsParent = Array.from(document.querySelectorAll('.tp-awards-vp-start-thumbs-wrapper .tp-awards-vp-start-move-thumb'));
			const moveThumbs = Array.from(document.querySelectorAll('.tp-awards-vp-start-thumbs-wrapper .tp-awards-vp-move-thumb-inner'));
			const overlappingThumbs = Array.from(document.querySelectorAll('.tp-awards-vp-end-thumbs-wrapper .tp-awards-vp-end-move-thumb'));
			...
			function animateElements() {
				moveThumbs.forEach((moveThumb, index) => {
					if (!overlappingThumbs[index] || !moveThumbsParent[index]) return;
					...
					createdTweens.push(moveAnimation);
				...
				if (startThumbsCaption) {
					const captionTween = gsap.to(startThumbsCaption, {
						scrollTrigger: {
					...
					createdTweens.push(captionTween);
					if (captionTween.scrollTrigger) createdTriggers.push(captionTween.scrollTrigger);
				}
			}

			function buildAwardsAnim() {
				animateElements();
				ScrollTrigger.refresh();
			}

			if (typeof imagesLoaded === 'function') {
				imagesLoaded(moveThumbsWrapper, buildAwardsAnim);
			} else {
				windowOn.on('load', buildAwardsAnim);
			}

			return () => {
				createdTriggers.forEach((t) => t && t.kill());
				createdTweens.forEach((t) => t && t.kill());
				moveThumbs.forEach((moveThumb, index) => {
					gsap.set(moveThumb, { clearProps: 'all' });
					if (moveThumbsParent[index]) moveThumbsParent[index].appendChild(moveThumb);
				});
				if (startThumbsCaption) gsap.set(startThumbsCaption, { clearProps: 'all' });
				ScrollTrigger.refresh();
			};
		});
	}
